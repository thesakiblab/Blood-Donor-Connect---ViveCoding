 ...
 */
// FIX: Changed `const` to `let` for `hexcase` to allow reassignment. The original code attempts to assign a default value in a catch block.
let hexcase = 0;   /* hex output format. 0 - lowercase; 1 - uppercase        */
// FIX: Changed `const` to `let` for `b64pad` to allow reassignment. The original code attempts to assign a default value in a catch block.
...
function hex_hmac_md5(k: string, d: string): string { return rstr2hex(rstr_hmac_md5(str2rstr_utf8(k), str2rstr_utf8(d))); }
function b64_hmac_md5(k: string, d: string): string { return rstr2b64(rstr_hmac_md5(str2rstr_utf8(k), str2rstr_utf8(d))); }
function any_hmac_md5(k: string, d: string, e: string): string { return rstr2any(rstr_hmac_md5(str2rstr_utf8(k), str2rstr_utf8(d)), e); }
...
function rstr2any(input: string, encoding: string): string
{
  ...
  const i_val = Array(Math.ceil(input.length / 4));
  for(let i = 0; i < i_val.length; i++)
  {
    i_val[i] = (input.charCodeAt(i * 4) << 24)
              | (input.charCodeAt(i * 4 + 1) << 16)
              | (input.charCodeAt(i * 4 + 2) << 8)
              | (input.charCodeAt(i * 4 + 3));
  }

  let output = "";
  while(i_val.length > 0)
  {
    const quotient = [];
    let remainder = 0;
    for(let i = 0; i < i_val.length; i++)
    {
      remainder = (remainder << 32) + i_val[i];
      if(remainder >= divisor)
      {
        quotient.push(Math.floor(remainder / divisor));
        remainder %= divisor;
      } else if (quotient.length > 0) {
        quotient.push(0);
      }
    }
    output = encoding.charAt(remainder) + output;
    // i_val = quotient;
  }
  return output;
}
